 ...
 */
export function exportCsv(samples, mode, isVisibleOnly = false) {
  ...
  for (const s of samples) {
    ...
    lines.push(
      `${s.t_ms},${s.ref.toFixed(2)},${s.medida.toFixed(2)},${uStr}`
    );
  }
  ...
  const prefix  = isVisibleOnly ? 'dados-controle-visivel' : 'dados-controle';

  const link = document.createElement('a');
  link.setAttribute('href', url);
  link.setAttribute('download', `${prefix}-${measureHeader}-${dateStr}.csv`);
